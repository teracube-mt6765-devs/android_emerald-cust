...
trees_data = {
    "device": {
        "url": "https://github.com/" + gh_org + "/" + "android_device_" + organization + "_" + codename,
...
if (tree["uses_prebuilt_kernel"] == True):
    ...
    print("Target has specified custom kernel usage. Checking additional repos for a kernel repository...")
    if ("kernel" in tree["additional_trees"]):
        print("Kernel repo found, appending as is...")
        trees_data.update({
            "kernel": tree["additional_trees"]["kernel"]
        })
else:
    ...
    trees_data.update({
        "kernel": {
            "url": name,
            "path": "kernel/" + organization + "/" + codename
        }
    })
...
if (tree["clone_sepolicy"] == True):
    if ("sepolicy" in tree["additional_trees"]):
        ...
        trees_data.update({
            "sepolicy": tree["additional_trees"]["sepolicy"]
        })
    else:
